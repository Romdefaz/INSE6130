...
#valid_rules="Container Working Directory in Proc Fd, Terminal shell in container, YeahAnothreOne"
...
#if [ "${valid_rules##*$rule_name*}" != "$valid_rules" ]; then
if [ "${valid_pri##*$priority*}" != "$valid_pri" ]; then
	...
	#echo "[$(date)] ℹ️  Could not find matching rules in valid_rules" >> /var/log/falco-actions.log  
